...
test("publications Abs toggle opens and closes", async ({ page }) => {
  ...
  test.skip((await absButton.count()) === 0, "no bibliography rendered on the publications page");
...
test("navbar search button opens modal and toggle buttons use pointer cursor", async ({ page }, testInfo) => {
  ...
  test.skip((await page.locator("#search-toggle").count()) === 0, "search is disabled on this site");
